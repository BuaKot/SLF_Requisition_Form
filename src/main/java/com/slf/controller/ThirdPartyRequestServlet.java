...
public class ThirdPartyRequestServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (loggedInEmpId(request) == null) {
            response.sendRedirect(request.getContextPath() + "/login");
            return;
        }
        try {
            int ownerEmpId = loggedInEmpId(request).intValue();
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            long linkId = linkDAO.createLink(
                ...
                FormRegistry.THIRD_PARTY_USER_REGISTRATION,
    ...
    private void cancelRequest(HttpServletRequest request, HttpServletResponse response, int ownerEmpId)
        ...
        if (cancelled) {
            auditDAO.log("THIRD_PARTY_REQUEST_CANCELLED", Long.valueOf(requestId), Long.valueOf(linkId),
                Integer.valueOf(ownerEmpId), "INTERNAL", "Internal owner cancelled third-party external form link.");
        }
