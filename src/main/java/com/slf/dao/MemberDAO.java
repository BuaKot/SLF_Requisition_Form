...
public class MemberDAO {
    ...
    public List<MemberProfile> findMembers(String sectionId, String position, String status) throws SQLException {
        ...
        try (Connection conn = DBConnection.getConnection()) {
            ...
            boolean hasEmailColumn = hasColumn(conn, "EMPLOYEE", "EMAIL");
            boolean hasEmailNotificationColumn = hasColumn(conn, "EMPLOYEE", "EMAIL_NOTIFICATION_ENABLED");
            ...
            String sql = buildMemberListSql(hasActiveColumn, hasEmailColumn, hasEmailNotificationColumn, filterSection, filterPosition, filterStatus);
            try (PreparedStatement ps = conn.prepareStatement(sql)) {
                ...
                try (ResultSet rs = ps.executeQuery()) {
                    while (rs.next()) {
                        ...
                        member.setEmail(hasEmailColumn ? rs.getString("EMAIL") : "");
                        member.setEmailNotificationEnabled(rs.getInt("EMAIL_NOTIFICATION_ENABLED") == 1);
    ...
    public MemberProfile findMemberById(int empId) throws SQLException {
        try (Connection conn = DBConnection.getConnection()) {
            ...
            String emailSelect = hasEmailColumn ? "e.EMAIL" : "NULL AS EMAIL";
            String emailNotificationSelect = hasEmailNotificationColumn
                ? "NVL(e.EMAIL_NOTIFICATION_ENABLED, 1) AS EMAIL_NOTIFICATION_ENABLED"
                : "1 AS EMAIL_NOTIFICATION_ENABLED";
            String sql =
                "SELECT e.EMPID, e.EMPNAME, e.POSITION, e.SECID, e.PHONE, " + emailSelect + ", " +
                "s.SECNAME, d.DEPTID, d.DEPTNAME, " + activeSelect + ", " + emailNotificationSelect + " " +
                "FROM EMPLOYEE e " +
    ...
    public void addMember(int empId, String empName, String position, int secId, String phone, String email,
        ...
        try (Connection conn = DBConnection.getConnection()) {
            boolean hasActiveColumn = hasColumn(conn, "EMPLOYEE", "IS_ACTIVE");
            boolean hasEmailColumn = hasColumn(conn, "EMPLOYEE", "EMAIL");
            StringBuilder columns = new StringBuilder("EMPID, EMPNAME, POSITION, SECID, PHONE");
            ...
            String sql = "INSERT INTO EMPLOYEE (" + columns + ") VALUES (" + values + ")";
            try (PreparedStatement ps = conn.prepareStatement(sql)) {
                int index = 1;
    ...
    public void updateMember(int empId, String empName, String position, int secId, String phone, String email,
                             String password, boolean active)
            throws SQLException {
        validateEmail(email);
        try (Connection conn = DBConnection.getConnection()) {
    ...
    public void updateEmailNotificationSettings(int empId, String email, boolean enabled) throws SQLException {
        validateEmail(email);
        try (Connection conn = DBConnection.getConnection()) {
            if (!hasColumn(conn, "EMPLOYEE", "EMAIL")) {
                throw new SQLException("EMPLOYEE.EMAIL column is required for mail notifications.");
            }
            if (!hasColumn(conn, "EMPLOYEE", "EMAIL_NOTIFICATION_ENABLED")) {
                throw new SQLException("EMPLOYEE.EMAIL_NOTIFICATION_ENABLED column is required for notification settings.");
            }
            try (PreparedStatement ps = conn.prepareStatement(
                    "UPDATE EMPLOYEE SET EMAIL = ?, EMAIL_NOTIFICATION_ENABLED = ? WHERE EMPID = ?")) {
    ...
    static String buildMemberListSql(boolean hasActiveColumn, boolean filterSection, boolean filterPosition, boolean filterStatus) {
        return buildMemberListSql(hasActiveColumn, true, true, filterSection, filterPosition, filterStatus);
    }

    static String buildMemberListSql(boolean hasActiveColumn, boolean hasEmailColumn, boolean filterSection, boolean filterPosition, boolean filterStatus) {
        return buildMemberListSql(hasActiveColumn, hasEmailColumn, true, filterSection, filterPosition, filterStatus);
    }

    static String buildMemberListSql(boolean hasActiveColumn, boolean hasEmailColumn, boolean hasEmailNotificationColumn, boolean filterSection, boolean filterPosition, boolean filterStatus) {
        String activeSelect = hasActiveColumn ? "NVL(e.IS_ACTIVE, 1) AS IS_ACTIVE" : "1 AS IS_ACTIVE";
        String emailSelect = hasEmailColumn ? "e.EMAIL" : "NULL AS EMAIL";
        String emailNotificationSelect = hasEmailNotificationColumn
            ? "NVL(e.EMAIL_NOTIFICATION_ENABLED, 1) AS EMAIL_NOTIFICATION_ENABLED"
        ...
        sql.append(emailSelect).append(", ");
        sql.append("s.SECNAME, d.DEPTID, d.DEPTNAME, ").append(activeSelect).append(", ");
        sql.append(emailNotificationSelect).append(" ");
    ...
    private static void validateEmail(String email) throws SQLException {
        if (!hasText(email)) {
            throw new SQLException("Email is required for mail notifications.");
        }
        String trimmed = email.trim();
        if (trimmed.length() > 255 || !trimmed.matches("^[^@\\s]+@[^@\\s]+\\.[^@\\s]+$")) {
            throw new SQLException("Email format is invalid.");
        }
    }
